	...
	private boolean fixIntersections() {
		for(int i = 0; i < coords.size(); i++) {
			...
			for(int j = 0; j < coords.size();j++)
			{
				...
				int count = 0;
				if(cor_1_i==cor_2_i) count++;
				if(cor_1_i==cor_3_i) count++;
				if(cor_1_i==cor_4_i) count++;
				
				if(cor_2_i==cor_3_i) count++;
				if(cor_2_i==cor_4_i) count++;
				
				if(cor_3_i==cor_4_i) count++;
				...
				//if(Line2D.linesIntersect(tmpX_1,tmpY_1, tmpX_2,tmpY_2, tmpX_3,tmpY_3,tmpX_4,tmpY_4) && cor_2_i != cor_3_i) {
					

					//System.out.println("Last fix sum was: "+lastFixes.get(lastFixes.size()-1));
					...
					/*if(lastFixes.size()==0) {
						...
						if(alreadyFixed) {
							System.out.println("Im done fixing this");
							return false;
						} else {
							alreadyFixed=true;
							System.out.println("I'll go one more iteration but im out after that one.");
							continue;
						}
					} else {
						alreadyFixed = false;
					...
					/*
					int half = cor_2_i + ((cor_3_i + 1) - cor_2_i) / 2;
					int endCount = cor_3_i;
					for (int startCount = cor_2_i; startCount < half; startCount++) {

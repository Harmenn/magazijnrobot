...
public class Bruteforce extends Algoritme {
    ...
    private void bruteforce(ArrayList<Product> in, int currentPosition) {
        if (currentPosition >= in.size()) { // bereikt laatste product, laatste keer proberen
        ...
        for (Bin bin : bins) {
            if (bin.addProduct(currentItem, true)) { //checkt als er ruimte is voor een product
                try {
                    bruteforce(in, currentPosition + 1); // weer opnieuw proberen
